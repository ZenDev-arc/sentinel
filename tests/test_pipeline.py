...
class TestGraphTopology:
    ...
    def test_all_required_nodes_present(self):
        ...
        required = {
            ...
            "review_style", "review_architecture", "lead_review",
            "generate_tests", "run_tests", "coverage",
            "integration_tests", "reproduce_bugs", "root_cause",
            "propose_fixes", "verify_fixes", "explain",
            "approval_gate", "finalise",
        }
...
class TestConditionalRouting:
    ...
    def test_route_to_coverage_on_no_failures(self):
        ...
        state = state.model_copy(update={
            "test_results": [TestResult(module="all", passed=10, failed=0)]
        })
    ...
    def test_route_to_bug_squad_on_failures(self):
        ...
        state = _make_initial_state()
        state = state.model_copy(update={
            "test_results": [TestResult(module="all", passed=8, failed=2, failing_tests=["test_foo"])]
        })
    ...
    def test_route_integration_tests_medium_risk(self):
        ...
        state = _make_initial_state()
        state = state.model_copy(update={
            "risk": RiskScore(level=RiskLevel.MEDIUM, score=0.5)
        })
    ...
    def test_skip_integration_tests_low_risk(self):
        ...
        state = _make_initial_state()
        state = state.model_copy(update={
            "risk": RiskScore(level=RiskLevel.LOW, score=0.1)
        })
...
class TestApprovalGate:
    def test_auto_merge_small_safe_fix(self):
        ...
        from src.core.state import ProposedFix, FixClassification

        state = _make_initial_state()
        state = state.model_copy(update={
            "risk": RiskScore(level=RiskLevel.MEDIUM, score=0.45)
        })
        fix = ProposedFix(
    ...
    def test_human_required_for_auth_fix(self):
        ...
        state = _make_initial_state()
        state = state.model_copy(update={
            "risk": RiskScore(level=RiskLevel.HIGH, score=0.80)
        })
        fix = ProposedFix(
