    ...
    agents: [
      {
        ...
        detail: 'runs a regex pre-scan for hardcoded secrets before any llm call. then uses the strong model to find injection risks (sql, command, ldap, template), auth/authorization flaws, unsafe deserialization, weak crypto, ssrf, path traversal. also covers js/ts-specific issues: xss via dangerouslySetInnerHTML, eval() with user input, localStorage token storage, missing csrf, and unvalidated redirects.',
      ...
      },
      {
        ...
        detail: 'analyses added/modified lines for n+1 query patterns in orm loops, unbounded queries without pagination, blocking sync i/o inside async functions, o(n²) algorithms, string concatenation in loops, and repeated identical db calls without caching. for js/ts: missing usememo/usecallback, setstate in loops, large arrays without memoization, missing react key props, and synchronous event-loop blocking.',
      ...
      },
      {
        ...
        detail: 'checks naming (snake_case, pascalcase, screaming_snake), bare excepts, mutable defaults, import*, boolean parameter flags, functions over 40 lines, missing type annotations, magic numbers, commented-out code, and todos without ticket references. for js/ts: var vs const/let, implicit any, == vs ===, console.log in prod, unhandled promises, missing useEffect deps, prop mutation, and missing error boundaries.',
        inputs: ['diff', 'kb codebase patterns'],
        outputs: ['style_findings: list[reviewfinding]'],
      },
      {
        name: 'architecture agent',
        role: 'checks design and module boundaries',
        detail: 'skipped only for trivially small diffs (< 10 additions). looks for layering violations, circular dependencies, god classes, tight coupling, feature envy, missing abstractions, public api changes without deprecation paths, and business logic leaking into the data layer.',
        inputs: ['diff', 'kb architecture patterns'],
