...
def _derive_patterns(runs: list[dict], kb_entries: list) -> list[dict]:
    ...
    if category_totals:
        ...
        if per_pr >= 1.0:
            patterns.append({
                ...
                "evidence": {"category": top_cat, "total_findings": top_count, "avg_per_pr": per_pr},
                "detected_at": now,
            })
    ...
    if regression_rate >= 10:
        patterns.append({
            ...
            "evidence": {"regression_rate_pct": regression_rate, "affected_runs": runs_with_regressions},
            "detected_at": now,
        })
    ...
    if avg_findings >= 5:
        patterns.append({
            ...
            "severity": "medium",
            "evidence": {"avg_findings_per_pr": avg_findings, "total_findings": total_findings},
            "detected_at": now,
        })
    ...
    if len(recent) >= 5:
        high_rate = sum(1 for r in recent if r.get("risk_level") == "high") / len(recent)
        if high_rate >= 0.4:
            patterns.append({
                ...
                "evidence": {"high_risk_rate_pct": round(high_rate * 100), "sample_size": len(recent)},
                "detected_at": now,
            })
